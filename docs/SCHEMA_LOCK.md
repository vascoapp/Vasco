...
**Version:** 1.22 — updated 2026-09-24 (erasure record survives the account: account_deletion_requests FK dropped)
<!-- v1.22 (migration 20260924000002): EXPORT, THEN DELETE (user's decision 2026-09-24). account_deletion_requests.user_id loses its FK to auth.users (was ON DELETE CASCADE — the Art. 17 completion record vanished with the user it recorded). The column stays uuid NOT NULL, now a bare id. drain-account-deletions nulls `reason` on completion (free text is not part of the minimal record) and deletes `done` rows older than 3 years every run. No anonymised business records are kept: invoices etc. go with the account; keeping them is the CONTRACTOR's duty (export first, app/contractor/delete-account.tsx). 🔴 Apply BEFORE deploying the worker. + 20260924000004: owner indexes for the worker's explicit deletes (analytics_events.user_id, job_embeddings.user_id, model_training_pairs.user_id, material_price_history.observed_by). Prev: -->
<!-- v1.21 (migrations 20260924000001 + 20260924000003): ANALYTICS. NEW TABLE analytics_events (id text PK minted on device ≤80, name ≤80, properties jsonb ≤4 KB, user_id uuid FK→auth.users ON DELETE SET NULL, user_role, country, session_id, platform, platform_version (all length-bounded), "timestamp", created_at) — eventTrackingService had flushed into it while it did not exist (42P01, swallowed). WRITE-ONLY: authenticated INSERT with check (user_id IS NULL OR user_id = auth.uid()); NO SELECT — so the client must use a plain INSERT (an upsert's ON CONFLICT needs SELECT → 42501; the first client did that). anon nothing. VERIFIED LIVE by behaviour: anonymous insert OK, foreign user_id refused by RLS, select 42501. get_analytics_summary(int) SECURITY DEFINER, service_role ONLY (was authenticated: platform totals to every contractor, while the PIN-gated admin page — anon — could not call it). NEW age_analytics_events() service_role only + pg_cron job `vasco-analytics-aging` (02:15 UTC): strips user/session/country/role after 12 months (the privacy screen's promise), deletes after 25. Prev: -->
<!-- v1.20 (migrations 20260819000011 + 20260819000012): SELLER FISCAL IDENTITY + THE ACCEPTANCE MIRROR. business_settings += province, fiscal_regime, person_type (all nullable, CHECK on person_type IN F|J). Needed because the ES/IT mappers cannot be written without them: FatturaPA requires CedentePrestatore RegimeFiscale and there is NO defaultable value — RF01 on a forfettario is a fiscally WRONG invoice that SDI ACCEPTS, which is worse than a rejection because nobody finds out; Facturae requires PersonTypeCode on both parties; both need provincia, and `state` is the US state field with different values and validation so overloading it would make a Texan and a Milanese share a column. ONE generic `fiscal_regime` rather than regime_fiscale + regimen_fiscal: a contractor operates in one country, so two columns would leave one permanently NULL. ⚠️ BEHAVIOUR CHANGE, not just schema: decide_acceptance_link now ALSO mirrors the decision onto documents.status. Accepting previously wrote only to quote_acceptance_links, and getAcceptanceStatus() — the sole contractor-side reader of that table — has ZERO callers, so a customer's acceptance landed in a table nobody opened: the quote still read 'sent', the contractor kept chasing, the customer believed they had answered. Every contractor surface already reads documents.status, so the decision is written THERE, on the one function every acceptance route passes through (web portal, /accept link, app). Vocabularies already matched (documents allows accepted|rejected; the link CHECK allows exactly those two; QuoteStatus spells them the same) so there is no mapping table to drift. Matched on user_id + document_number + doc_type='quote' — quote_id on the link is TEXT holding the document_number, and matching on it alone would be a CROSS-TENANT write. Never walks a 'paid' quote backwards. The mirror is wrapped in its own EXCEPTION block: losing the customer's decision because a badge update failed would be worse than a stale badge. ALSO: verify-quote-token now returns an `acceptance` capability (looked-up-before-minted) so the signed quote portal can accept — it grants nothing new, since only the holder of a valid HMAC reaches that line. Prev: -->
<!-- v1.19 (migrations 20260819000007 + 20260819000010): PRICE OBSERVATIONS caught up (see the drift note below) and CUSTOMERS gained the address a structured e-invoice needs. customers += city, postcode, country, province, vat_id, tax_id, einvoice_routing, einvoice_email — all nullable, all additive. WHY: app/invoices/[id].tsx passed `(invoice as any).customerCity`/`.customerPostcode`, fields that existed NOWHERE, so XRechnung omitted CityName and PostalZone and BR-DE-8/9 require both — every German invoice was invalid on the buyer address alone. Facturae needs buyer NIF + province + post code; FatturaPA needs P.IVA or Codice Fiscale + CAP + comune + provincia + Codice Destinatario, without which SDI rejects (00311/00312) and in Italy the invoice was never legally issued. GENERIC, NOT PER-COUNTRY: vat_id carries USt-IdNr / NIF / P.IVA / BTW; tax_id the second identifier some markets need alongside it (Codice Fiscale — an Italian consumer has a CF and no P.IVA); einvoice_routing whatever addresses the buyer's receiving channel (IT Codice Destinatario, '0000000' when routing by PEC; a Peppol participant ID later). Six near-empty country columns would rot. country NULL = "same as the contractor", which keeps every pre-existing row correct rather than silently German. ⚠️ NEW WRITE MAPPER customerUpdatesToRowPayload — until now every Customer field was spelled the same in both worlds so updateCustomer passed the patch straight through; `vatId` breaks that and PostgREST rejects the WHOLE statement on one unknown column, so a single camelCase key would have killed every customer edit touching it. Both the online call and the queued offline payload go through it. Splitting the existing free-text `address` was never an option: "Marktplatz 3, 10178 Berlin" and "Via Roma 1 — 20100 Milano (MI)" share no grammar. Prev: -->
<!-- v1.18 (migrations 20260819000005 + 20260819000006): RETURN-SHAPE CHANGE, additive. get_acceptance_link_by_token(text) and decide_acceptance_link(text,text,text) now also return `contractor_name` and `contractor_country`, resolved SERVER-SIDE from the row's user_id — which is still never returned. Needed because /accept/[token] stopped being a deep-link bouncer and now renders and accepts the quote: the business name is what makes a capability link arriving cold in a WhatsApp message legible, and the country decides the currency (a Dutch contractor's quote is in euros whichever phone opens it — learnings #151/#157). Existing keys unchanged, so a client that has not shipped yet keeps working. HARDENING: record_portal_event and log_portal_activity now wrap their INSERT in an EXCEPTION block returning false. customer_portal_events.event_type carries a CHECK of twelve values and the function validated only LENGTH, so a thirteenth propagated 23514 out of a function whose COMMENT promises it "returns false instead of raising — telemetry must not break the customer's screen". The mobile app ships on its own cadence and can learn an event name before the database does. Caught rather than re-listing the twelve values in a second place: a duplicate registry drifts in both directions at once (learnings #170). Prev: -->
