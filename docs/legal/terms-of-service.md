# Terms of Service — Vasco

**Last updated:** March 24, 2026
**Effective date:** March 24, 2026

These Terms of Service ("Terms") govern your use of the Vasco mobile application ("App") operated by Vasco ("we," "us," "our"). By downloading, installing, or using the App, you agree to these Terms.

---

## 1. Service Description

Vasco is an AI-powered business management platform for construction trades professionals. The App provides:
- Job scheduling and management
- Quote and invoice creation with PDF generation
- AI-powered business insights and automation
- Time tracking and materials management
- Customer relationship management
- Compliance and certification tracking
- Payment processing integration (via Mollie)
- Accounting software integration

---

## 2. Account Registration

### 2.1 Eligibility
You must be at least 16 years old and legally authorized to conduct business in your country to use the App.

### 2.2 Account Security
You are responsible for maintaining the confidentiality of your account credentials. You must notify us immediately of any unauthorized use of your account.

### 2.3 Accurate Information
You agree to provide accurate, current, and complete business information. You are responsible for keeping this information up to date.

---

## 3. Acceptable Use

You agree NOT to:
- Use the App for illegal activities
- Submit false or misleading business information
- Attempt to access other users' data
- Reverse engineer, decompile, or disassemble the App
- Use the App to send spam or unsolicited communications
- Interfere with or disrupt the App's infrastructure
- Use automated systems (bots) to interact with the App
- Share your account credentials with unauthorized individuals

---

## 4. AI-Powered Features

### 4.1 AI Recommendations
The App provides AI-generated insights, predictions, and automation suggestions. These are advisory in nature and should not be relied upon as professional financial, legal, or business advice.

### 4.2 Human Approval Required
All customer-facing actions (invoices, reminders, follow-ups, quotes) require your explicit approval before execution. The AI prepares work; you decide whether to proceed.

### 4.3 Accuracy Disclaimer
AI predictions (quote win probability, job duration estimates, payment timing) are based on historical data and statistical models. Actual outcomes may differ. We do not guarantee the accuracy of any AI-generated output.

### 4.4 Photo Analysis
When you use the invoice scanning feature, photos are processed by AI to extract pricing data. You are responsible for verifying the accuracy of extracted information before using it.

---

## 5. Financial Features

### 5.1 Invoicing and Quotes
The App helps you create and manage quotes and invoices. You are solely responsible for:
- The accuracy of amounts, descriptions, and VAT calculations
- Compliance with local invoicing regulations
- Proper record-keeping as required by tax authorities

### 5.2 Payment Processing
Payment processing is provided by Mollie B.V. Use of payment features is subject to Mollie's terms of service (https://www.mollie.com/terms). We are not responsible for payment processing failures, chargebacks, or disputes.

### 5.3 Accounting Integrations
When you connect third-party accounting software, data synchronization is provided on a best-effort basis. You are responsible for verifying data accuracy in your accounting system.

### 5.4 Tax Calculations
VAT and tax calculations provided by the App are estimates based on standard rates. You are responsible for ensuring tax compliance. Consult a qualified tax professional for specific advice.

---

## 6. Data Ownership

### 6.1 Your Data
You retain full ownership of all business data you enter into the App, including:
- Customer records
- Job details
- Quotes and invoices
- Photos and documents
- Time tracking entries

### 6.2 Data Export
You may export your data at any time in standard formats (CSV, PDF).

### 6.3 Data on Account Deletion
Upon account deletion, we will delete your personal data within 30 days, except where retention is required by law (see Privacy Policy for details).

### 6.4 License Grant
You grant us a limited, non-exclusive license to use your data solely for the purpose of providing and improving the App's services, including AI model training on anonymized, aggregated data.

---

## 7. Intellectual Property

### 7.1 Our Property
The App, including its design, features, AI models, code, and branding, is owned by Vasco and protected by intellectual property laws.

### 7.2 Trademarks
"Vasco," the Vasco logo, and related marks are trademarks of Vasco. You may not use these marks without written permission.

### 7.3 Feedback
If you provide feedback or suggestions about the App, we may use them without obligation to you.

---

## 8. Service Availability

### 8.1 Uptime
We aim to provide reliable service but do not guarantee uninterrupted availability. The App works offline for core features.

### 8.2 Maintenance
We may temporarily suspend the App for maintenance, updates, or security patches. We will provide reasonable notice when possible.

### 8.3 Changes to Features
We reserve the right to modify, add, or remove features at any time. Material changes will be communicated in advance.

---

## 9. Subscription and Pricing

### 9.1 Free and Paid Tiers
The App may offer both free and paid subscription tiers. Pricing and feature availability for each tier will be clearly communicated within the App.

### 9.2 Billing
Paid subscriptions are billed through the Apple App Store or Google Play Store, subject to their respective terms and refund policies.

### 9.3 Cancellation
You may cancel your subscription at any time through your app store account. Access to paid features continues until the end of the current billing period.

### 9.4 Price Changes
We may change subscription prices with 30 days' advance notice. Continued use after a price change constitutes acceptance.

---

## 10. Limitation of Liability

### 10.1 Disclaimer
THE APP IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.

### 10.2 Limitation
TO THE MAXIMUM EXTENT PERMITTED BY LAW, VASCO SHALL NOT BE LIABLE FOR:
- Loss of data, revenue, profits, or business opportunities
- Indirect, incidental, special, or consequential damages
- Damages arising from AI recommendations or predictions
- Payment processing errors by third-party providers
- Tax calculation inaccuracies
- Service interruptions or downtime

### 10.3 Maximum Liability
Our total liability to you shall not exceed the amount you paid for the App in the 12 months preceding the claim.

---

## 11. Indemnification

You agree to indemnify and hold Vasco harmless from any claims, damages, or expenses arising from:
- Your use of the App
- Your violation of these Terms
- Your violation of any third-party rights
- Inaccurate information you provide

---

## 12. Dispute Resolution

### 12.1 Governing Law
These Terms are governed by the laws of the Netherlands.

### 12.2 Jurisdiction
Disputes shall be resolved in the courts of Amsterdam, the Netherlands, unless applicable consumer protection law provides otherwise.

### 12.3 Informal Resolution
Before filing a formal complaint, you agree to contact us at support@vasco.dev to attempt informal resolution.

---

## 13. Termination

### 13.1 By You
You may stop using the App and delete your account at any time through the App settings.

### 13.2 By Us
We may suspend or terminate your account if you:
- Violate these Terms
- Use the App for illegal purposes
- Fail to pay subscription fees (if applicable)
- Provide false information

### 13.3 Effect of Termination
Upon termination, your right to use the App ceases. We will retain your data as described in our Privacy Policy.

---

## 14. Miscellaneous

### 14.1 Entire Agreement
These Terms, together with the Privacy Policy, constitute the entire agreement between you and Vasco.

### 14.2 Severability
If any provision of these Terms is found unenforceable, the remaining provisions remain in effect.

### 14.3 No Waiver
Our failure to enforce any right or provision does not constitute a waiver of that right.

### 14.4 Assignment
We may assign our rights under these Terms. You may not assign your rights without our written consent.

---

## 15. Contact Us

For questions about these Terms:

**Email:** support@vasco.dev
**Address:** [Your registered business address]
**Website:** https://vasco.dev
