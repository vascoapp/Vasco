/**
 * ES, walked as a ES contractor. One country per file — see euWalk.tsx for why.
 */
import { walkCountry, DUTCH_REGISTRY } from '../src/test-utils/euWalk';

describe('ES contractor surface', () => {
  let report: any[] = [];

  it('mounts every core screen', async () => {
    report = await walkCountry('fontanero', 'ES');
  ...
  it('reaches its OWN tax authority on the compliance screen', () => {
    ...
    expect(certs?.texts.join(' | ')).toContain('Agencia Tributaria');
  });
});
