...
/** Routes _layout.tsx sends every signed-in user away from — read from the
 *  single list in src/config/dormant.ts so the map and the gate agree. */
const DORMANT_SRC = readFileSync(join(ROOT, 'src/config/dormant.ts'), 'utf8');
const DORMANT_ROUTES = [...DORMANT_SRC.slice(DORMANT_SRC.indexOf('DORMANT_ROUTES'), DORMANT_SRC.indexOf('};', DORMANT_SRC.indexOf('DORMANT_ROUTES'))).matchAll(/^\s*'?([\w()/-]+)'?\s*:/gm)].map((m) => m[1]);
const REDIRECTED = DORMANT_ROUTES;
...
function routeOf(file) {
  ...
  const joined = parts.filter((p) => p !== 'index').join('/');
  return { file, segs, groups, redirected: REDIRECTED.some((d) => joined === d || joined.startsWith(`${d}/`)) };
}
...
function importsOf(file) {
  const src = read(file);
  const out = [];
  // import x from '…' · import '…' (side effect) · import('…') · require('…') · export … from '…'
  for (const m of src.matchAll(/(?:import\s[^'"]*?from\s*|import\s*(?=['"])|import\s*\(\s*|require\s*\(\s*|export\s[^'"]*?from\s*)['"]([^'"]+)['"]/g)) {
...
const layoutFiles = walk(APP).filter((f) => /\/_layout\./.test(f) && !REDIRECTED.some((g) => relative(APP, f).startsWith(`${g}/`)));
...
if (WRITE_MANIFEST) {
  const files = unreached.map((f) => relative(ROOT, f)).sort();
  const out = join(ROOT, 'src/config/dormant.files.json');
  const { writeFileSync } = await import('fs');
  writeFileSync(out, JSON.stringify({ note: 'GENERATED by scripts/reachability.mjs --write-manifest. Dormant = unreachable by a signed-in contractor; see src/config/dormant.ts.', files }, null, 1) + '\n');
  console.log(`dormant manifest: ${files.length} files → src/config/dormant.files.json`);
} else if (AS_JSON) {
