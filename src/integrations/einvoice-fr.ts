...
//
// ⚠️ STATUS (2026-08-30). The XML half is correct and SHIPPING, but it is
// generated by `generateFacturXXML` in ./einvoice.ts, not by this file —
// `generateFacturXXml` below has no callers. The bug that mattered was the
// guideline URN: France was being handed `urn:cen.eu:en16931:2017` (plain
// ZUGFeRD/EN 16931), which is not a Factur-X profile, so the file failed
// Factur-X validation while the UI called it Factur-X. Fixed + pinned by
// src/integrations/__tests__/facturXProfile.test.ts.
//
// 🔴 STILL MISSING — the PDF/A-3 container. A complete Factur-X invoice is ONE
// file: a PDF/A-3 with the CII XML embedded as `factur-x.xml`, AFRelationship
// /Alternative, listed in the document-level /AF array, plus XMP metadata
// declaring the Factur-X conformance level. We ship the XML on its own.
//
// This cannot be done from the current pipeline, and it is worth being precise
// about why rather than reaching for a PDF library:
//   - invoice PDFs come from `expo-print` (HTML -> PDF). Its output is not
//     PDF/A and cannot be made so after the fact: PDF/A-3 requires an
//     OutputIntent with an embedded ICC profile, and EVERY font embedded and
//     subset. A post-hoc attach gives a hybrid PDF that carries the XML but
//     still fails PDF/A validation — which is worse than the honest split,
//     because it LOOKS conformant.
//   - the fix belongs server-side: render the PDF with a PDF/A-capable
//     generator in an edge function, attach the XML there, return one file.
//
// Until then the contractor gets a valid Factur-X XML they can file, and the
// PDF separately. Do not claim PDF/A-3 in the UI.
// =============================================================================
