  ...
  return useMemo(() => {
    ...
    if (bestJobType && worstJobType && bestJobType.effectiveHourlyRate > worstJobType.effectiveHourlyRate * 1.3) {
      ...
      insights.push({
        ...
        type: 'correlation',
        priority: 'high',
        title: `${bestJobType.jobType} levert ${formatMoney(rateGap)}/uur meer op`,
        description: `${bestJobType.jobType} (${formatMoney(bestJobType.effectiveHourlyRate)}/uur) vs ${worstJobType.jobType} (${formatMoney(worstJobType.effectiveHourlyRate)}/uur). Meer focus op ${bestJobType.jobType} verhoogt je gemiddeld uurtarief.`,
    ...
    if (dsoOverTarget > 0 && collections.summary.totalOutstanding > 3000) {
      ...
      insights.push({
        ...
        title: `Langzame betaling kost ${formatMoney(workingCapitalCost)}/maand werkkapitaal`,
    ...
    if (travelWaste > 300 && idleWaste > 300) {
      insights.push({
        ...
        type: 'correlation',
        priority: 'medium',
        title: `Reistijd + idle = ${formatMoney(travelWaste + idleWaste)}/maand verlies`,
        description: `Reiskosten (${formatMoney(travelWaste)}) en idle-tijd (${formatMoney(idleWaste)}) samen reduceren door route-clustering en betere planning. Potentieel: ${formatMoney(laborCosts.monthlyOptimizationPotential)}/maand.`,
    ...
    const connections: ServiceConnection[] = [
      ...
      },
      {
        ...
        to: 'laborCostService',
        relationship: `${formatMoney(supplierNeg.totalDiscountPotential)} onbenut korting potentieel`,
        strength: Math.min(88, Math.round(supplierNeg.totalDiscountPotential / 20)),
      },
      {
        from: 'laborCostService',
        to: 'scheduleService',
        relationship: `${formatMoney(laborCosts.monthlyOptimizationPotential)} optimalisatiepotentieel`,
