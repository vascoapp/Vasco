...
//   • Payment CTA          — opens portalData.paymentLink when present (the base
//                            RPC doesn't return it yet, so it stays hidden until
//                            BE adds paymentLink/paymentStatus — same as mobile).
//
...
interface QAEntry { id: string; q: string; a: string | null; pending: boolean }
...
export default function CustomerPortal({ params }: PageProps) {
  ...
  const sendQuestion = useCallback(async () => {
    ...
    setQa(prev => [...prev, { id, q, a: null, pending: true }]);
    setQDraft('');
    const sb = getSupabase();
    const demo = data.accessToken.length < 8;
    let answer = 'Thanks — your contractor will reply soon.';
    if (sb && !demo) {
      try {
        ...
        const r = res as { autoReply?: string } | null;
        if (r?.autoReply) answer = r.autoReply;
      } catch { /* keep default */ }
    }
    setQa(prev => prev.map(e => e.id === id ? { ...e, a: answer, pending: false } : e));
    setQSending(false);
  }, [qDraft, data, lang]);
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
              {qa.map(e => (
                <div key={e.id}>
                  ...
                  {e.pending ? <div style={{ fontSize: 12, color: DK.muted, padding: '6px 12px' }}>…</div>
