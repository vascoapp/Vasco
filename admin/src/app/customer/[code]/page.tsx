        ...
        {data.categories.map(cat => (
          <section key={cat.id} style={{ marginTop: 24 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
              {/* `get_portal_by_access_code` returns `'name', category` — the raw id. So
                  this heading read "CAT_BATH_FIXTURES" (the CSS uppercases it) on the
                  page the CUSTOMER opens, in every market. The id is stable, so it
                  resolves against the app's own catalogue translations. */}
              <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', color: DK.muted }}>{portalCategoryName(lang as PortalLang, cat.id, cat.name)}</h3>
              ...
              return (
                <article key={item.id} style={{ background: DK.panel, border: `1px solid ${DK.border}`, borderRadius: 14, padding: 16, marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <div style={{ flex: 1 }}>
                      {/* `decision_items.label` stores the ENGLISH source string and carries no
                          catalog id, so the RPC's `'name', di.label` is English for every
                          customer. Resolved by that English label + the category, which
                          also works for trackers already in the database. */}
                      <h4 style={{ margin: '0 0 3px', fontSize: 15, fontWeight: 700 }}>{portalItemName(lang as PortalLang, cat.id, item.name)}</h4>
                  ...
                  {decided && !isEditing ? (
                    <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ flex: 1, background: DK.panel2, borderRadius: 10, padding: '10px 12px' }}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 600, marginTop: 2 }}>{labelFor(item, lang as PortalLang, cat.id, t)}</div>
                      </div>
                            ...
                            <button key={o.value} disabled={pendingItem === item.id} onClick={() => void submit(item, o.value)}
                              ...
                              <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                ...
                                <span style={{ textAlign: 'left' }}>
                                  <span style={{ display: 'block', fontSize: 14, fontWeight: 600 }}>{portalOptionLabel(lang as PortalLang, cat.id, item.name, o.value, o.label)}</span>
...
/**
 * The customer's own answer, read back to them. Two things were English here
 * regardless of market: the stored option label, and the boolean literals.
 */
function labelFor(item: PortalItem, lang: PortalLang, categoryId: string, t: Record<string, string>): string {
  const v = item.value;
  if (v == null) return '—';
  const opt = (item.options ?? []).find(o => o.value === String(v));
  if (opt) return portalOptionLabel(lang, categoryId, item.name, opt.value, opt.label);
